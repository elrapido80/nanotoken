...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "nanotoken:"))
                fCommandLine = true;
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 19578 or testnet: 19995)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9578)") + "\n" +
...
//    }
    ...
    // Make sure only a single nanotoken process is using the data directory.
    ...
    // requested to kill nanotoken-qt during the last operation. If so, exit.
