...
// Copyright (c) 2011-2014 Nanotoken Developers.
// Copyright (c) 2013-2014 Dr Kimoto Chan
// nanotoken by r3wt and camodude009
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                ...
                if (pindexLast->nHeight+1 >= 125000) { DiffMode = 2; }
        }
...
{
        static const int64        BlocksTargetSpacing                        = 1.5 * 60; // 1.5 minutes/90 seconds
        unsigned int                TimeDaySeconds                                = 60 * 60 * 24;
        int64                                PastSecondsMin                                = TimeDaySeconds * 0.08;
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "nanotoken", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // nanotokend: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
